...
- **渲染 / 动画 / 客户端资源**：本阶段已完成 bedrock model/render 基础设施的大块迁移，已落地内容包括：
   ...
   - **2026-03-09 第一人称 follow-up 收口（TRIS-dyna 检视飞天 / 枪焰贴图污染 / 曳光缺失）**：针对用户以 `run/tacz/[Tacz1.1.5+]TRIS-dyna GunsPack ver1.1.5.zip.zip` 为基准追加的 4 个可见问题，本轮继续对齐上游 `CustomInterpolator.java`、第一人称动画运行链、`MuzzleFlashRender` 与 `EntityBulletRenderer` 的可观察结果，并补齐 1.12.2 的必要接线差异。
      ...
      - 曳光运行时验收已使用 `trisdyna:cms92` 收口：`build/smoke-tests/runclient-focused-smoke-20260309-001920.log` 中可见连续 marker：`BULLET_CLIENT_SPAWN ... tracer=true shooterResolved=true`、`BULLET_RENDERER_ACTIVE ...`、`TRACER_SKIP_CLOSE_RANGE ...`（近距离保护仍保留）以及随后多条 `TRACER_RENDERED gun=trisdyna:cms92 ammo=trisdyna:emx_b1 firstPerson=true color=#deffd7 ...`。截图清单 `build/smoke-tests/focused-smoke-screenshots/runclient-focused-smoke-20260309-001920/01-muzzle_0.png`、`02-spawn_0.png`、`03-render_0.png`、`04-tracer_0.png` 中，`04-tracer_0.png` 已人工复核可见第一人称绿色曳光束，证明这条链路已经真正回到可见运行态，而不是只在日志里“理论成立”。
         ...
         - **第六轮 reopen：弹速倍率越高 tracer 越乱 / 客户端物理同步与帧间插值多因素收口**：用户反馈"弹速倍率 ≤ 0.3 时 tracer 正常，大于 0.3 就开始乱飞，越大情况越明显"。测试武器 ch104 (TRIS-dyna pack)。根因分析：
           1. **`prevRotation = rotation` 杀死帧间旋转插值（主因）**：`onUpdate()` 在 `lerpRotation(prev, target)` 之后手动设 `prevRotationYaw = rotationYaw; prevRotationPitch = rotationPitch`。这导致渲染时 `prevRotation == rotation`，`lerp(pt, prev, cur)` 无论 partialTicks 是多少都返回相同值——帧间没有旋转插值。每 tick 的旋转跳到 lerp 后的新值。高速子弹每 tick motion 角度变化更大，0.2 lerp 因子留 80% 残差，跳变越明显。上游 TACZ 的 `tick()` 在 `super.tick()` → `Entity.tick()` 中由引擎在 tick **开头**设 `xRotO = xRot`，之后不再手动设 prev；Legacy 应与此一致，由 `super.onEntityUpdate()` 在 tick 开头设 prev。
           2. **`.tracker(64, 1, true)` 发送速度同步包（次因）**：上游 `EntityType.Builder.setShouldReceiveVelocityUpdates(false)` 禁止发送速度包。Legacy 的 `.tracker(64, 1, true)` 使 `EntityTrackerEntry` 每 tick 比较 motion 差异并发 `SPacketEntityVelocity`，`EntityThrowable.setVelocity()` 覆写客户端 `motionX/Y/Z`。服务端 motion 已经过 friction/gravity，客户端 `onUpdate()` 再次 friction/gravity → 双重衰减。
           3. **客户端缺少 gravity/friction/lifespan 同步**：`writeSpawnData`/`readSpawnData` 从未同步这些字段。客户端使用默认值 `bulletGravity=0.0f, friction=0.01f`，与服务端的 `BulletCombatData` 实际值不一致 → 客户端物理轨迹与服务端偏离。
           4. **硬编码 `return 0.3`**：`resolveFocusedSmokeBulletSpeedMultiplier()` 入口硬编码了 `return 0.3`，后续的系统属性读取代码不可达。
           - **修正措施**：
             1. `LegacyEntities.kt`：删除 `prevRotationPitch = rotationPitch; prevRotationYaw = rotationYaw`，恢复帧间插值。
             2. `LegacyEntities.kt`：`.tracker(64, 1, true)` → `.tracker(64, 1, false)`，匹配上游 `setShouldReceiveVelocityUpdates(false)`。
             3. `LegacyEntities.kt`：新增 `setVelocity()` override 为 no-op，防止残余速度包干扰。
             4. `LegacyEntities.kt`：`writeSpawnData`/`readSpawnData` 增加 `bulletGravity` / `friction` / `lifespan` 同步。
             5. `LegacyEntities.kt`：删除 `resolveFocusedSmokeBulletSpeedMultiplier()` 中的硬编码 `return 0.3`。
           - 验证：`./gradlew classes` + `./gradlew test` 全量通过。
- **验证状态**：编译与定向测试已覆盖 Client UX / gunsmith / refit backend / render parsing 等阶段性变更；渲染基础设施已有一轮成功的 `runClient` smoke。2026-03-08 最终清理轮已移除此前阻塞 `compileTestKotlin` 的 `LegacyClientShootCoordinatorBurstStateTest`（引用已废弃的 pending-burst API）与 `MathUtilFovTest`（引用已移除的 FOV 函数），以及无法在无头环境运行的 `BedrockGunModelRefitViewPathTest`（`ClassNotFoundException`），同时修复 `TACZAudioRuntimeTest.manifest rejects ogg files with incomplete vorbis headers`（改为使用 channels=0 使 probe 真正拒绝）。
   - 2026-03-09 本轮继续修复 4 项 gameplay parity 缺口并新增到 219 tests 全通过：
     ...
     4. **客户端动画状态机时间戳（hmg22 连射加速）**：`GunAnimationStateContext.getLastShootTimestamp()` / `getShootTimestamp()` 此前返回服务端相对时间戳（`ShooterDataHolder.lastShootTimestamp`），而 `getCurrentTimestamp()` 返回 `System.currentTimeMillis()`，导致 delta 总是巨大、连射检测永远不成立。现已改为从 `LegacyClientShootCoordinator` 读取客户端绝对时间戳（`clientLastShootTimestampMs` / `clientShootTimestampMs`），使 Lua 脚本中的 `context:getCurrentTimestamp() - context:getLastShootTimestamp() < shootInterval + 100` 判定回到正确区间。hmg22 的客户端 state machine 脚本（`applied_armorer:moritz_mg_hmg22`）与服务端 data script（`applied_armorer:moritz_mg_hmg22_logic`）两条加速路径现均有正确的时间基准。
       5. **枪械 HUD display 解析改为 stack-aware（`GunDisplayId` override parity）**：上游 `TimelessAPI.getGunDisplay(stack)` 会先按 `ItemStack` 上的 `GunDisplayId` 解析 display，再回退到 gun-index 默认 display；Legacy 之前的 HUD 与客户端动画驱动却只按 `gunId` 走 `TACZGunPackPresentation.resolveGunDisplayId(...)`，会把同一枪包里本应显示 `NORMAL` 的枪误判成其它 display 的 `PERCENT` 样式。现已在 `LegacyRuntimeTooltipSupport.resolveGunDisplayId(...)` 中补齐 stack-aware 解析：优先读取并校验 `GunDisplayId` NBT，缺失/默认 sentinel/无效值时再回退到 gun-index 默认 display；`LegacyClientOverlayEventHandler.kt` 与 `LegacyClientGunAnimationDriver.kt` 也统一改走这条解析链。新增 `LegacyRuntimeTooltipSupportTest` 覆盖 override / fallback / 空栈场景。运行时验证中，`trisdyna:s10dmd` 已正确解析到 `trisdyna:s10dmd_display`，focused smoke `build/smoke-tests/runclient-focused-smoke-20260310-130704.log` 记录 `HUD_AMMO ... style=normal currentText=096`，归档截图 `build/smoke-tests/focused-smoke-screenshots/runclient-focused-smoke-20260310-130704/01-hud_ready.png` 也已人工复核右下 HUD 为数值 `096` 而非百分比。
      6. **2026-03-10 HUD / ASCII / spread / recoil parity 收口**：`LegacyClientOverlayEventHandler.kt` 已把右下枪械 HUD 从共享的 `LegacyInputExtraCheck.isInGame()` 门禁中拆出，改为只要求 `player/world` 存在且未按 `F1` 隐藏 GUI，因此 `GuiChat` 等界面打开时 HUD 仍会按上游持续显示；focused smoke HUD 诊断日志也新增 `screen=` 维度，便于后续专门验收“GUI 下 HUD 仍可见”。ASCII 路径则已从全局 `FontRenderer` mixin 回退为显式 helper：新增 `client/foundation/TACZAsciiFontHelper.java`，在 `LegacyClientOverlayEventHandler.kt`、`GunSmithTableScreen.kt`、`GunRefitScreen.kt`、`TextShowRender.java` 以及相关 item overlay / tooltip 调用点对**纯 ASCII** 文本只在外层临时执行 `fontRenderer.setUnicodeFlag(false)`，渲染/量宽完成后立即恢复原 flag；`mixins.tacz.json` 中的 `FontRendererMixin` 已移除，避免继续全局侵入 `FontRenderer`。战斗侧则新增 `TACZGunPropertyResolver.kt` 与 `TACZBulletSpreadResolver.kt`，把此前只在 `GunRefitScreen.kt` 里消费的 `fire_mode_adjust`、heat `min/max_inaccuracy`、`crawl_recoil_multiplier`、attachment `inaccuracy/recoil` modifier、递归 attachment tag 匹配（含 `tacz:intrinsic/slug`）真正接回 `LivingEntityShoot.kt` 与 `TACZGunScriptAPI.kt`：现在默认射击链和 Lua `calcSpread` 路径都会根据姿态 / 热量 / 配件计算真实散布，slug 弹匣会把霰弹 `bullet_amount` 压成 1 发而不再误判为独头弹缺失。`heat min/max_rpm_mod` 本轮也已补进 `GunDataAccessor.kt` 与 `TACZGunScriptAPI.kt` getter，作为后续射速 parity 的公共数据基础，但尚未改变当前发射 cadence。客户端侧新增 `TACZCameraRecoilHandler.kt`，由 `LegacyClientShootCoordinator.kt` 在本地击发确认后初始化 camera recoil 样条，并通过 `ServerMessageGunFire.kt` 跳过本地玩家回声，避免和服务端广播重复叠加。除既有 `TACZGunPropertyResolverTest` 外，本轮还新增 `TACZAsciiFontHelperTest` 覆盖 ASCII 判定与 flag 恢复语义；`./gradlew test --no-daemon` 已通过。运行时方面，focused smoke `build/smoke-tests/runclient-focused-smoke-20260310-144035.log` 已成功打到 `[FocusedSmoke] HUD_AMMO ... style=normal ... reserveText=0240` 标记，归档截图 `build/smoke-tests/focused-smoke-screenshots/runclient-focused-smoke-20260310-144035/01-hud_ready.png` 也已人工复核右下 HUD 的 ASCII 弹药数字清晰可见；该 smoke 随后因既有 `reload_timing_delta_-1311` 断言结束为 FAIL，但这不影响 HUD/ASCII helper 路径已实际跑通的结论。
   - 当前 `./gradlew test` 全量通过（已包含本轮新增的 `TACZGunPropertyResolverTest`），focused smoke 的历史 PASS 证据仍覆盖 `tacz:timeless50`（半自动 fire-mode fallback）、`tacz:b93r`（三连发服务端调度）、`tacz:rpg7`（爆炸弹道）、`trisdyna:cms92x` / `trisdyna:ch104` / `tacz:s10dmd`（fuel/inventory 弹药检查）等主链。本轮为 HUD-under-GUI 与 camera recoil 追加的新 smoke 截图尝试，则被 Forge 1.12 对多版本 Kotlin jar 的 ASM 扫描问题挡在模组初始化前；这应继续记录为**环境阻塞**，而不是把已通过的主源码编译/单测结论误判成本轮功能回退。
...
因此，下一阶段最值得投入的主线通常不是“继续重迁数据/战斗/Client UX/Render 基础设施主链”，而是：
    1. **Render 剩余子轨**：~~animation state machine~~（已落地）、~~关键帧插值~~（已落地）、bone animation application、ammo/attachment display renderer、~~muzzle flash~~（本轮已落地） / shell ejection、第一人称 hand/scope 渲染链路、~~程序化后坐力 / 射击摆动 / 跳跃摆动~~（本轮已落地）、~~ADS 二阶动力学平滑~~（本轮已落地）、~~约束骨骼逆变换~~（本轮已落地）
      2. **Combat 剩余 parity 子轨**：~~hurt/kill 事件与客户端同步消息~~（已完成，详见下方命中/击杀反馈）、~~边界 fire-mode 状态切换~~、~~needCheckAmmo 真值~~（已修复）、~~burst 服务端调度~~（已补齐 `BurstFireTaskScheduler.tick()` 到 `ServerTickEvent`）、~~脚本武器时间戳基准~~（已修复 `GunAnimationStateContext` 客户端绝对时间戳）、~~AmmoCountStyle HUD~~（已迁移）、~~stack-aware GunDisplayId HUD/动画 display 解析~~（已补齐 `LegacyRuntimeTooltipSupport.resolveGunDisplayId(...)`，HUD 与客户端动画驱动不再只按 `gunId` 误取 display，`trisdyna:s10dmd` runtime HUD 已回到数值 `096`）、~~过热系统散热加速 + 冷却延迟对齐~~（已修复 `tacz$tickHeat` 不再重置 `heatTimestamp`，公式与上游完全一致；`handleShootHeat` 补上 `heatMax` cap；`heatTimestamp` 改为在 `shoot()` 入口统一赋值，脚本路径也覆盖）、~~过热数据客户端-服务端不同步~~（已修复 `tacz$tickHeat` 加 `isRemote` 保护，仅服务端计算冷却，客户端通过 ItemStack NBT 同步获取，与上游 `onTickServerSide` 仅服务端 tick 一致；根因是客户端 `heatTimestamp` 仅在 `draw()` 时设置而不随射击更新，导致客户端冷却量远大于服务端）、~~客户端自动拉拴缺失~~（已补齐 `tickClientAutoBolt()` 到 `LegacyClientPlayerGunBridge.onClientTick()`，与上游 `LocalPlayerBolt.tickAutoBolt()` + `bolt()` 行为一致：每 tick 检测 MANUAL_ACTION 枪膛内无弹 + 有弹药可用时自动触发拉拴、发包、播放动画/音效，并在 `hasBulletInBarrel` 同步到位后清除 `isBolting` 状态；修复了 m700、k30 等手动拉拴武器需要额外点击才能拉拴的问题）以及少数未覆盖武器脚本/门禁边角的一致性收尾
         - **2026-03-10 命中/击杀反馈系统（Hit/Kill Feedback）**：完整移植上游 `ClientHitMark.java`、`RenderCrosshairEvent.java`（hit marker 渲染）、`KillAmountOverlay.java`（kill amount HUD）、`ServerMessageGunHurt.java`、`ServerMessageGunKill.java`（S2C 事件消息）、`EntityHurtByGunEvent`、`EntityKillByGunEvent`（事件模型）以及 `SoundPlayManager` 中 `playHeadHitSound/playFleshHitSound/playKillSound` 三个命中音效路径。落地文件：`GunEvents.kt`（新增 `EntityHurtByGunEvent`/`EntityKillByGunEvent`）、`ServerMessageGunHurt.kt`/`ServerMessageGunKill.kt`（S2C 消息，含 `addScheduledTask` 线程安全转发）、`TACZNetworkHandler.kt`（注册两条新消息）、`TACZClientGunSoundCoordinator.kt`（新增三个命中/击杀音效方法）、`GunDisplayInstance.java`（`checkSounds()` 中 `putIfAbsent` 注入 `head_hit`/`flesh_hit`/`kill` 默认音效键）、`LegacyHitFeedbackState.kt`（客户端状态机：命中/爆头/击杀时间戳 + kill streak 计数 + 300ms 淡出 + kill amount overlay 带 HSB 渐变与淡出 alpha）、`LegacyClientHitMarkHandler.kt`（客户端事件处理：hurt→markHit + headshot 检测 + 音效 + TargetMinecart 伤害计数器；kill→markKill + 音效；focused smoke 日志发射 `HIT_FEEDBACK_TRIGGERED`/`KILL_FEEDBACK_TRIGGERED`）、`LegacyClientOverlayEventHandler.kt`（hit marker 四象限 16×16 贴图渲染 + kill amount 0.5x 居中渲染）、`LegacyEntities.kt`（`EntityKineticBullet` 新增 `HitFeedbackResult` + `emitHitFeedback()`，使用 `!target.isEntityAlive` 判定击杀并发布事件/S2C 消息；`appliedDamage` 前置守卫防止后续弹命中已死亡实体时重复触发 kill 事件）、`ClientProxy.kt`（注册 `LegacyClientHitMarkHandler`）。`FocusedSmokeRuntime.kt` 新增 hit feedback target 生成（`EntityPigZombie` + `setNoAI(true)`），`scripts/runclient_focused_smoke.sh` 新增 `FOCUSED_SMOKE_SPAWN_HIT_TARGET` 环境变量映射。单元测试：`LegacyHitFeedbackStateTest`（状态机时序/kill streak/headshot）+ `GunDisplayInstanceHitFeedbackSoundTest`（默认音效键注入）；`./gradlew test` 全量 PASS。Focused smoke 运行时确认：`HIT_FEEDBACK_TARGET_READY entityId=174 health=3.75` → `KILL_FEEDBACK_TRIGGERED killAmount=1`（三连发仅 1 次 kill 事件，去重逻辑正确）→ `PASS`。
