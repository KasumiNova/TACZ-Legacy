...
- 这条线的**本阶段迭代已完成**，当前 `TACZ-Legacy` 中已有 `IGunOperator.kt`、`ShootResult.kt`、`ReloadState.kt`、`KnockBackModifier.kt`、`FireMode.kt`、`ShooterTickHandler.kt`、`common/entity/shooter/**`、`TACZNetworkHandler.kt`、`common/network/message/**`、`LivingEntityMixin.java`，并已补上 ammo 搜索 parity、缺失的 S2C 消息类型与基础客户端事件投递链路。
- 现在使用这个 Prompt 时，默认应视为**回归修复 / 协议缺口补齐 / 剩余 parity 收尾**，而不是继续把 shooter/network 主链当成本阶段主攻方向。
- `Client UX` 的本阶段迭代也已完成；若任务重点是 HUD / 音效 / 动画 / overlay / 输入反馈，或 `gun_smith_table` 等界面对这些事件与状态的消费，优先改用 `client ux` 或 `render animation` Prompt。
...
- HeatSystem、crawl、tracer 等后续战斗能力，默认视为下一轮明确需求，而不是当前阶段的自动延伸
